...
//ankita
	...
	protected void onBind() {
		...
		((FocusWidget) display.getStateTextBox()).addBlurHandler(blurHandler);
	...
	//	display.getTestButton().addClickHandler(clickHandler);
		...
		((FocusWidget) display.getBankNameTextBox()).addBlurHandler(blurHandler);
		((FocusWidget)display.getHolderNameTextBox()).addBlurHandler(blurHandler);
		((FocusWidget)display.getIfscTextBox()).addBlurHandler(blurHandler);
		((FocusWidget)display.getCityTextBoxForPayment()).addBlurHandler(blurHandler);
		((FocusWidget)display.getStateTextBoxForPayment()).addBlurHandler(blurHandler);
		//ankita
		/**
		 * For GetStartedButton .
		 */
...
//______________________________________________________________________________________
		...
		((FocusWidget) display.getStateTextBox()).addFocusHandler(focusHandler);
		...
		((FocusWidget) display.getPhoneNumberTextBox())
				.addFocusHandler(focusHandler);
//payment mode 
		((FocusWidget) display.getChequeTextBox()).addFocusHandler(focusHandler);
		((FocusWidget)display.getAccNoTextBox()).addFocusHandler(focusHandler);
		((FocusWidget)display.getBankNameTextBox()).addFocusHandler(focusHandler);
		((FocusWidget)display.getHolderNameTextBox()).addFocusHandler(focusHandler);
		((FocusWidget)display.getIfscTextBox()).addFocusHandler(focusHandler);
		((FocusWidget)display.getCityTextBoxForPayment()).addFocusHandler(focusHandler);
		((FocusWidget)display.getStateTextBoxForPayment()).addFocusHandler(focusHandler);
		//ankita
	...
	public void getFieldsOnFocus(boolean way) {
		if(way){
			String[] id = {"chequeTextBox","accNoTextBox","bankNameTextBox","holderNameTextBox","ifscTextBox","cityTextBox","stateTextBox"};
			getFocusStatus(id);
	...
	public void doValidation(String[] id) {
		...
		for (int i = 0; i < id.length; i++) {
			...
			if (id[i].equalsIgnoreCase("firstNameTextBox")) {
				...
				stateName = display.getStateTextBoxForPayment().getValue().trim();
			...
			diseaseName = display.getdiseaseNameTextBox()
					.getValue().trim();
...
//doctor name
			doctorName = display.getdoctorNameTextBox().getValue()
					.trim();
